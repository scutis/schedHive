...
mysql.connect(end, function(connection) {

    console.log("Setting up database...");
    connection.query('DROP TABLE IF EXISTS user, g_list, g_member, g_thread, t_comment, t_file, t_sched, t_pref, notif, p_table');
    ...
    connection.query('CREATE TABLE t_sched (id INT unsigned PRIMARY KEY AUTO_INCREMENT, t_id INT unsigned, s_type TINYINT, s_from TIMESTAMP, s_to TIMESTAMP)');
    ...
    fs.readFile(__dirname + '/user_db.csv', {
        ...
        parseCSV(csvData, {}, function(err, output) {
            ...
            connection.query('INSERT INTO user SET ?', mod, function(){
                connection.release();
                end();
            });
